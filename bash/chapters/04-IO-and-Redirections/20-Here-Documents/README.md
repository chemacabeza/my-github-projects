...
cd: cd [-L|[-P [-e]] [-@]] [dir]
    ...
    ...
    Exit Status:
    Returns 0 if the directory is changed, and if <strong>/home/username/Repositories/somerepo/_bash-in-depth/chapters/04-IO-and-Redirections/20-Here-Documents/script</strong> is set successfully when
    -P is used; non-zero otherwise.
...
```bash
#!/usr/bin/env bash
#Script: heredoc-0006.sh
cat <<EOF | base64 -d
RXhhbXBsZSBvZiBsaW5lCg==
EOF
```
...
```bash
#!/usr/bin/env bash
#Script: heredoc-0010.sh
: <<COMMENT
This is a comment for a piece
of code that seems to be very complex
blah blah blah
COMMENT
echo "Script with HereDoc comment"
```
